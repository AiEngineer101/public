(function() {
    ...
    const config = {
      ...
      subtitle: "Here to help owners and residents! " + sessionId,
      primaryColor: "#e83e2f",
      initialMessage: "His there! 👋 How can I help you today?",
      position: "bottom-right", // "bottom-right" or "bottom-left"
      webhookUrl: "https://westrom-agent-991313418129.us-central1.run.app", // New configuration option for webhook URL
      sessionId: sessionId // Generate a unique session ID
    };
    // set global variable
    var escalationCompleted = 0;
  
  //   setInterval(async () => {
  //     const res = await fetch('https://aisolv.app.n8n.cloud/webhook/bc27c663-5765-47fb-b7e6-8b73dae5e952');
  //     const data = await res.json();
  //     if (data.new) addBotMessage(data.text);
  //   }, 3000);
    ...
    async function generateBotResponse(userMessage) {
        try {
          const eventSource = new EventSource(`https://westrom-agent-991313418129.us-central1.run.app/stream?prompt=${encodeURIComponent(userMessage)}&sessionId=${encodeURIComponent(config.sessionId)}`);
          let responseText = '';
          let lastBotMessageElement = null;
      
          eventSource.onmessage = function (event) {
            if (event.data.includes("[DONE]")) {
              console.log("escalationCompleted", event.data);
              eventSource.close();
              // enable the send button once complete generation is done
              if(escalationCompleted == 0) {
            ...
            if(event.data.includes("Thank you for your request. We will contact you shortly!")) {
                console.log("escalated!")
                const sendButton = document.getElementById("sendButton");
                sendButton.style.cursor = 'default';
                sendButton.style.opacity = '0.5';
                sendButton.disabled = true;
                const input = document.querySelector('.wp-whisper-chat-input input');
                input.disabled = true;
                input.value = "Ask another question by refreshing the pages";
                escalationCompleted = 1;
            }
            ...
            if (lastMessage && !lastMessage.isUser) {
              // Just update the bubble HTML directly
              lastMessage.text = responseText;
              if (lastBotMessageElement) {
                lastBotMessageElement.querySelector('div').innerHTML = marked.parse(responseText);
              }
            } else {
              // First time bot responds — add a new bubble
              const newMessage = {
                text: responseText,
                isUser: false,
                timestamp: new Date()
          ...
          eventSource.onerror = function (error) {
            ...
            eventSource.close();
            const sendButton = document.getElementById("sendButton");
            if (sendButton && escalationCompleted == 0) {
              sendButton.style.cursor = 'pointer';
              sendButton.style.opacity = '1';
              sendButton.disabled = false;
            }
